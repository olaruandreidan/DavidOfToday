...
import { ScoreRadar, TrendChart } from '../components/ScoreVisuals'
import { streak } from '../domain/scoring'
...
export function DashboardScreen() {
  ...
  return <section className="py-5"><PageHeading eyebrow="Your compass" title="Meet the David of today."><p>One honest reflection at a time. The trend uses the final successful check-in from each local day.</p></PageHeading>
    ...
    <div className="grid gap-5 lg:grid-cols-2"><ScoreRadar baseline={state.baseline.scores} current={state.currentScores} /><TrendChart state={state} /></div>
    <Link className="button mt-6 w-full sm:w-auto" to="/daily">Start a daily check-in</Link>
  </section>
}
